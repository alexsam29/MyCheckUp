...
|   | Actor(s): Admin|           System              |
|---| ---------------|-------------------------------|
| 1 | From the dashboard or after the patient completes the survey (system recommends to book an appointment), the patient selects “book an in-person appointment”. | (*1) Displays the input form to book an appointment.  Form includes a calendar to select a date and time (only displays available time slots). |
